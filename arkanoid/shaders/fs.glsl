...
vec3 compLightDir() {
  // lights
  // Point
  vec3 pointLightDir = normalize(uLightPosition - fs_pos);
  // Direct
...
vec4 compLightColor(vec3 lightDir, vec4 lightCol) {
  ...
  // lights
  // Point
  vec4 pointLightCol = lightCol * pow(uLightTarget / length(uLightPosition - fs_pos), uLightDecay);
  // Direct
...
void main() {
    ...
    vec4 diffColor = vec4(uDiffuseColor,1.0) * texcol;
    vec4 ambMatColor = vec4(uColor,1.0) * texcol;
