...
var S3 = `
	OlightDir = normalize(Pos - fs_pos);
	OlightColor = lightColor * clamp((dot(OlightDir, Dir) - cos(radians(ConeOut/2.0)))/(cos(radians(ConeIn*ConeOut/2.0)) - cos(radians(ConeOut/2.0))), 0.0 , 1.0 );
...
var S5 = `
	OlightDir = normalize(Pos - fs_pos);
	OlightColor = lightColor * pow(	 Target / length(Pos - fs_pos), Decay) *
	clamp((dot(OlightDir, Dir) - cos(radians(ConeOut/2.0)))/(cos(radians(ConeIn*ConeOut/2.0)) - cos(radians(ConeOut/2.0))), 0.0 , 1.0 );
`;
